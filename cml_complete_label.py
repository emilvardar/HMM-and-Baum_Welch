...
if __name__ == '__main__':
    ...
    for example in range(5):
        ...
        A_init, B_init, _ = get_initial_values(A_gt, B_gt, 0.5)
        A, B, prob_list = fit(A_init, B_init, pi_gt, teoretical_observations, teoretical_states, ETA, epochs)
        ...
        print('A_diff:', np.linalg.norm(A-A_gt)/K)
        print('B_diff:', np.linalg.norm(B-B_gt)/np.sqrt(K*L))
        plt.plot(prob_list)
        plt.show()
