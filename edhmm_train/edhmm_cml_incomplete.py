...
initial distribution matrices for Explicit Duration Hidden Markov Models (EDHMM) using 
conditional maximum likelihood approach (CML) where the labels for the states are 
'incomplete'. Observe that we use normalization in this approach in order to be able 
to use longer seqeunces. 
...
@njit
def calculate_m_ia(m_il, B, S, D, observations):
    """
    Calculates m_i(a) so it is compatible with EDHMM. 
    """
    m_ia = np.zeros((S, B.shape[1]))
    for i in range(0, S*D, D):
            ...
            for l in range(m_il.shape[1]):
                if observations[l] == a:
                    for d in range(D):
                        temp += m_il[i+d, l]
            m_ia[i//D, a] = temp 
    return mia_converter(m_ia, B.shape[0], D, B.shape[1])

@njit
def mia_converter(m_ia, N, D, K):
    mia_new = np.zeros((N, K))
...
def update_1_epoch(A, B, P, pi, S, D, observations, labels, update_A, update_B, update_P, update_pi):
    """
    ...
    for r in range(len(observations)):
        ...
        if update_B or update_P or update_pi:
            ...
            if update_B:
                m_ia += calculate_m_ia(m_il_r, B, S, D, observations[r])
...
def fit(A, B, P, pi, S, D, observations, labels, max_epoch, update_A=True, update_B=True, update_P=True, update_pi=True):
    """
    ...
    for epoch in range(max_epoch):
        A, B, P, pi, prob_1_epoch, prob_2_epoch = update_1_epoch(A, B, P, pi, S, D, observations, labels, update_A, update_B, update_P, update_pi)                                                    
